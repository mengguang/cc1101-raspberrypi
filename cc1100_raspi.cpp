...
{
    extern volatile uint8_t SelfAddress;
...
uint8_t CC1100::rx_payload_burst(uint8_t rxbuffer[], uint8_t &pktlen)
{
    ...
    else
    {
        if (debug_level > 0)
        {
            printf("no bytes in RX buffer or RX Overflow!: ");
            printf("0x%02X \r\n", bytes_in_RXFIFO);
        }
        sidle(); //set to IDLE
        spi_write_strobe(SFRX);
...
void CC1100::spi_begin(void)
{
    ...
    //printf ("init SPI bus... ");
    cs_high();
    if ((x = wiringPiSPISetup(0, 2000000)) < 0) //4MHz SPI speed
    {
        if (debug_level > 0)
        {
